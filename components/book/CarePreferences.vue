...
<template>
<div class="max-w-[467px] mx-auto my-[30px] sm:my-[50px] flex flex-col gap-5 px-2.5 sm:px-2">
	<div>
		<h1 class=" leading-5 sm:text-[24px] sm:leading-7 font-bold">Care Preferences</h1>
		<p class=" text-[#00000080]/50 text-xs sm:text-sm sm:leadin-[18px]">Preferences will be applied to all orders</p>
	</div>
		...
		<div class="">
			<div class="flex flex-wrap gap-2.5 ">
				<label v-for="v of customer.water_temperatures" :key="v" class="px-5 py-[14px] btn btn-outline  grow border-brand-black/20 [&:is(:hover,.active)]:bg-[#F85A47] [&:is(:hover,.active)]:!border-[#e5e5e5] [&:is(:hover,.active)]:text-white" :class="book.water_temperature===v && 'active'">
					...
					<p class="text-sm sm:text-[16px] font-medium leading-5">{{ v }}</p>
				</label>
			</div>
	...
	<div class="collapse  rounded-[5px]  ">
		...
		<div class="collapse-title text-sm sm:text-base  flex justify-between items-center py-[12px] px-5 font-medium border !border-gray-300 rounded-[5px]">
	...
	<div>
		<div class="mb-2.5 flex justify-between items-center">
			<p class="text-black text-base leading-5 sm:leading-7 sm:text-[24px] font-bold ">Preferences Note</p>
			<p class=" text-[10px] leading-[15px] font-medium sm:text-base cursor-pointer " @click="book.preference_note=''">Clear</p>
		</div>
	...
	<button :class="!book.water_temperature || !book.dryer_temperature? 'bg-[#F3F3F6] text-black' : 'bg-[#F85A47]  text-white' " class=" hidden sm:block  w-full py-[18px] px-5  rounded-[5px] text-2xl text-[16px] font-bold leading-6  cursor-pointer   " @click="next()">Continue</button>
</div>
</template>
<style scoped>
.detergent-input {
