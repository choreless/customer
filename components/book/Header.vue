...
<div class="sticky top-0  mx-auto shadow-md sm:shadow-none bg-white z-50 max-w-[120rem] ">
	<section class="py-2.5 px-2.5  sm:px-[3.125rem] sm:pt-5 sm:pb-4 mx-auto grid grid-cols-[1fr_auto] md:grid-cols-[1fr_1fr_1fr] items-center">
		<div class="flex items-center gap-2.5">
			<button v-if="book.step>0" class="btn btn-sm btn-ghost fill-black hover:bg-transparent" @click="--book.step">
				<IconArrow />
			</button>
			<p class="w-7 h-7 rounded-full font-bold leading-[0.875rem] flex items-center justify-center bg-[#d4e8e6] text-black">{{ book.step+1 }}</p>
			<p class="font-bold leading-7 text-2xl text-black  ms-2.5">{{ step_names[book.step] }}</p>
		</div>
	...
	<div class="w-full hidden sm:block border-gray-100 h-[0.5rem] bg-[#dbdbdb]">
		<div class="  h-[0.5rem] bg-[#d4e8e6] transition-width duration-500" :style="{width: `${book.bookProgress}%`}" />
	</div>
</div>
</template>
