				...
				<div class="flex flex-col items-start sm:gap-[1.875rem] gap-5">
					<div class="flex flex-col items-start sm:gap-5 gap-2.5">
						...
						<button v-for="date of book.pinned_pickup_dates" :key="date.formatted">
								<div class="sm:w-[6.688rem] sm:h-[7.875rem] w-auto h-auto px-2.5 py-[0.188rem] rounded-[0.313rem] border-[0.031rem] border-brand-stone flex flex-col items-center [&:is(:hover,.active)]:border-brand-black" :class="book.date===date.formatted && 'active'" @click="book.date=date.formatted">
			...
			<div v-show="show_calendar">
				<ClientOnly>
					<VCalendar borderless expanded class="" title-position="left" :first-day-of-week="2" :columns="app.breakpoints.sm ? 2 : 1" :min-date="book.pinned_pickup_dates[0].unformatted" :attributes="[{highlight: true, dates: parseISO(book.date)}]" @dayclick="selectDate" />
						...
						<div class="flex sm:p-2.5 px-2.5 py-[0.438rem] flex-col items-start sm:gap-2.5 gap-[0.313rem] self-stretch rounded-[0.313rem] bg-brand-frost-light">
							<div class="sm:text-sm text-xs leading-4 sm:leading-5">All deliveries are between 6pm and 10pm. Standard turnaround on all orders is 1-2 days. </div>
					...
					<div class="flex flex-col items-start sm:gap-5 gap-2.5 w-full">
						...
						<div v-for="(service, index) in book.selected_services" :key="index" class="flex flex-col gap-5 w-full">
							<div class="flex w-full" >
								<div class="sm:w-[6.688rem] sm:h-[7.875rem] w-auto h-auto px-5 py-[0.188rem] rounded-[0.313rem] rounded-r-none border-r-0 border-[0.031rem] border-brand-stone flex flex-col items-center">
								...
								<div class="flex w-[calc(100%-3.875rem)] sm:px-[1.875rem] sm:h-[7.875rem] h-auto px-5 py-2.5 justify-between items-center border-[0.031rem] border-brand-stone">
									<div class="flex sm:py-[2.438rem] py-0 flex-col items-start">
										<div class="font-medium sm:text-base sm:leading-6 text-sm leading-5 text-brand-black">{{service.service_name}}</div>
										<div class="sm:text-base text-sm leading-5 sm:leading-6 text-[#00000080]">{{ !service.service_speed && service.service_name==='Home & Bedding' ? '72h service' : service.service_speed ? '48h service' : '24h service' }}</div>
									</div>
				...
				<div class="flex flex-col items-start sm:gap-5 gap-2.5 w-full">
					...
					<div  class="flex  w-full items-start content-start gap-5 flex-wrap">		
						<div v-for="frequency of book.frequencies" :key="frequency" class=" [&:is(:hover,.active)]:bg-brand-black [&:is(:hover,.active)]:border-brand-stone [&:is(:hover,.active)]:shadow [&:is(:hover,.active)]:text-white px-5 py-2.5 rounded-[0.313rem] border-[0.063rem] border-black  text-black sm:font-medium font-normal whitespace-nowrap sm:leading-6 sm:max-w-[13.938rem] w-[45%] sm:h-12 h-10 flex items-center justify-center gap-2.5 flex-shrink-0 cursor-pointer sm:text-base text-sm leading-5" :class="frequency==book.frequency && 'active'" @click="book.frequency=frequency">{{frequency}}</div>
					</div>
					<button class=" bg-[#f8f8f8] px-5 py-5 flex w-full justify-center items-center rounded-[3.125rem] text-black font-bold " >Continue</button>
				</div>
			</div>
		</div>
		</div>
