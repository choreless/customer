...
function get_selected_Wash(){
	book.wash_type=selected_Wash.value
}
		...
		<div v-for="(item,index) of wash_services_data" :key="index" class=" relative px-[0.938rem] py-2.5 cursor-pointer rounded-[0.625rem] border-[0.032rem] border-[#f2f2f2] w-full flex flex-col gap-2.5" :class="book.wash_type==item.wash_type ? 'active' : ''" @click="select_service(item)">
			<div class="flex  items-center gap-5 self-stretch">
				...
				<div class="flex flex-col justify-between items-end self-stretch pt-5">
					<div class="flex py-[0.188rem] px-[0.938rem] gap-2.5 rounded-[1.25rem] border-[0.063rem] border-brand-black text-right text-sm font-medium" @click="update_screen('preferences', index)">
						+ Add
					</div>
					<div class="text-xs font-medium text-right " @click="update_screen('pricing', index)">See pricing</div>
				</div>
			</div>
			...
			<div v-else class="flex w-full flex-col items-start gap-5">
				...
				<div v-for="(item,index) in preferences" :key="index" class="flex flex-col items-start self-stretch gap-2.5">
					...
					<div class="flex items-start gap-2.5 w-full ">
						<div v-for="(sub_item,sub_index) in item.items" :key="sub_index" :class="sub_item.is_selected && '!bg-brand-black text-white' " class="px-5 py-[0.875rem] flex-1 text-center cursor-pointer bg-white rounded-[5px]  font-medium text-brand-black border-[1px] border-[#6f6e7433] " @click="select_preference(sub_item,index)">{{ sub_item.name }}</div>
					</div>
				</div>
