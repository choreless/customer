...
import Card from '~/components/dashboard-2/Card.vue';
import SearchAddress from '~/components/dashboard-2/SearchAddress.vue';
const dashboard=usePageDashboard()
function address_selected(address){
	dashboard.pickup_address=address;
}
</script>
...
<div class="px-2.5 my-[3.125rem] max-w-80 sm:max-w-[26.875rem] mx-auto w-full">
	<div class="flex flex-col gap-2.5 sm:gap-[1.25rem]">
		...
		<SearchAddress :address="dashboard.pickup_address" class="w-full sm:w-[20.625rem] " @get_address="address_selected" />
		<Card v-for="(order,index) in dashboard.order_data_pickup" :key="index" :index="index" :order_data="order" />
