...
const isFormValid = ()=>error.first_name===false && error.last_name===false && !(error.company_name !== undefined && error.company_name)
;

async function next(){
	if(customer_type.value === 'commercial') error.company_name = !company_name.value;
...
<div>
	<div class="max-w-[467px] px-2 mx-auto my-12">
		...
		<p class="h-auto py-[15px] px-5 input !input-bordered rounded-[0.3125rem] mt-2.5 text-[2rem] font-bold cursor-not-allowed !text-brand-black disabled:bg-transparent">{{ entry.phone.masked }}</p>
		<p class="text-xs mt-[10px] text-brand-black/50">Your order status will be updated via this phone number.</p>
		<hr class="my-[18px] border-[#000000] opacity-30 border-[0.5px] border-b-0">
		...
		<div class="flex items-center gap-x-16 mt-5">
			<label class="flex gap-x-2.5 items-center cursor-pointer hover:text-primary">
				<input v-model="customer_type" type="radio" name="customer_type" value="retail" class="checkbox checkbox-primary w-5 h-5 rounded-sm">
				<p>Individual</p>
			</label>
			<label class="flex gap-x-2.5 items-center cursor-pointer hover:text-primary">
				<input v-model="customer_type" type="radio" name="customer_type" value="commercial" class="checkbox checkbox-primary w-5 h-5 rounded-sm">
				<p>Company</p>
			</label>
		</div>
		<div v-if="customer_type==='commercial'" class="mt-5">
			<div class="grid grid-cols-[1fr_auto] justify-around items-center gap-x-2 border rounded-[0.3125rem] border-base-content/20">
				<label class="input-float">
					<input v-model="company_name" type="text" placeholder="" class="!border-0">
					<p>Company Name</p>
				</label>
				<div class="w-6 h-6 me-4">
					<img v-if="error.company_name" src="https://ik.imagekit.io/choreless/v2/icons/MaterialSymbolsCancelRounded.svg" alt="icon" loading="lazy">
					<img v-else-if="error.company_name===false" src="https://ik.imagekit.io/choreless/v2/icons/FigmaSuccess.svg" alt="icon" loading="lazy">
			...
			<p v-if="error.company_name" class="text-error">* Company name is required</p>
		</div>
		...
		<div class="mt-5">
			<div class="grid grid-cols-[1fr_auto] justify-around items-center gap-x-2 border rounded-[0.3125rem] border-base-content/20">
				<label class="input-float">
					<input v-model="referal" type="text" placeholder="" class="!border-0">
					<p>Referal Code</p>
				</label>
				<div class="w-6 h-6 me-4">
					<!-- <span v-if="error.email==='progress'" class="loading bg-primary" />
		...
		<div class="mt-5">
			<label class="flex cursor-pointer hover:text-primary">
				<input v-model="notifications" type="checkbox" name="customer_type" value="retail" class="checkbox checkbox-primary w-5 h-5 rounded-sm">
				<p class="text-xs ml-[10px]">I want to receive text updates about promos and specials to the phone number provided. Message & Data rates may apply. Message frequency varies. Text HELP for help and STOP to cancel.</p>
			</label>
		</div>
		<button :disabled="!isFormValid()" class="btn w-full h-[3.75rem] rounded-[0.3125rem] text-2xl px-5 py-[0.9375rem] mt-5 text-white bg-brand-orange border-brand-orange hover:text-brand-orange hover:bg-transparent hover:border-brand-orange" @click="next()">Continue</button>
	</div>
</div>
</template>
