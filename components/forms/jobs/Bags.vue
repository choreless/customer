	...
	<div v-for="(v, k) of bags" :key="k" class="grid items-center gap-x-2 mt-4" :class="{'grid-cols-[1fr_auto]': bags.length>1}">
		<div class="grid grid-cols-[1fr_auto] items-center justify-around gap-x-2 border border-base-content/20 rounded-[0.3125rem]">
			<label class="input-float">
				<input type="number" v-model.number="bags[k]" placeholder="" min='0' class="!border-0" />
				<p>Laundry bag {{k+1}} (lb)</p>
			</label>
		</div>
		<button v-if="bags.length>1" @click="deleteBag(k)" class="btn btn-square btn-outline border-none btn-error hover:!text-white"><Icon name="ic:baseline-delete-forever" class="text-3xl" /></button>
	</div>
	<p v-if="error_weight_computed" class="text-error">* Invalid weight</p>
	<div class="flex items-center justify-between">
		<button @click="bags.push(null)" class="btn btn-sm gap-x-1 mt-2 btn-primary text-gray-20">
