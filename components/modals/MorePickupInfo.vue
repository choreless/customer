...
const props = defineProps<{
  ...
  error: Error;
  showMorePickupInfoModal?: Boolean | false
}>()
const emit = defineEmits<{
  ...
  (e: 'hideMorePickupInfoModal'): void;
  (e: 'saveMorePickupInfo'): void;
}>();
...
<template>
<dialog ref="morePickupInfoModalRef" :class="`modal ${showMorePickupInfoModal ? 'modal-open' : ''}`">
	<div class="modal-box">
		<form method="dialog">
			<button class="btn btn-sm btn-circle btn-ghost absolute right-2 top-2" type="button" @click="$emit('hideMorePickupInfoModal')">✕</button>
		</form>
		...
		<p v-if="error.collection_type" class="text-error mt-1">* Collection type is required</p>
			...
			<button
				...
				@click="$emit('saveMorePickupInfo')"
			>
				Save and Continue
			</button>
		</div>
	</div>
	<form method="dialog" class="modal-backdrop" @click="$emit('hideMorePickupInfoModal')">
		<button>close</button>
	</form>
</dialog>
</template>
